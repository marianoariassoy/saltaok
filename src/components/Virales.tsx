  ...
  return (
    <section className="bg-black" id="virales">
      <div className="container max-w-6xl m-auto px-8 pt-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">{loading ? <Loader /> : data && data.map((item) => <ArticleVirales key={item.id} id={item.id} title={item.title} image={item.image} />)}</div>
      </div>
    </section>
  );
};
