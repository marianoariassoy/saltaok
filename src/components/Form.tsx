    ...
    <>
      {error ? (
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div>
            <input {...register("name", { required: true })} placeholder={text["es"].name} className="w-full h-11 p-3 mb-4 border rounded-md text-sm text-black" />
            {errors.name && <Error />}
          </div>

          <div>
            <input {...register("email", { required: true })} placeholder={text["es"].email} className="w-full h-11 p-3 mb-4 border rounded-md text-sm text-black" />
            {errors.email && <Error />}
          </div>

          <div>
            <textarea {...register("message", { required: true })} className="w-full p-3 h-32 mb-4 border rounded-md text-sm text-black" placeholder={text["es"].message} />
